...
if ($_GET['action'] == "course_tag") {		
	...
	*/
	if ($_GET['dir'] == "up") {
		$upd_query = "UPDATE course_tag SET course_tag_count=course_tag_count+1 WHERE course_tag_course_id=".$_GET['courseid']." AND course_tag_tag_id=".$_GET['tagid'].";";
		$upd = mysql_query($upd_query);
	} elseif ($_GET['dir'] == "dn") {
		$upd_query = "UPDATE course_tag SET course_tag_count=course_tag_count-1 WHERE course_tag_course_id=".$_GET['courseid']." AND course_tag_tag_id=".$_GET['tagid'].";";
		$upd = mysql_query($upd_query);			
	...
	$my_url = "index.html?courseid=".$_GET['courseid'];

} else {

	$my_url = "index.html?error=true";
